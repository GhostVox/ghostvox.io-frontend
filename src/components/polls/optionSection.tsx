    ...
    return (
        <div className="mt-2 mb-4 space-y-2">
            ...
            <span className="font-medium">
              {option.name}
                {poll.userVote?.OptionID === option.id && isDetailView && (
                    <span
                        className="ml-2 text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300 px-2 py-0.5 rounded-full">
                  Your vote
                </span>
                )}
            </span>
            ...
            <div className="flex  gap-4 items-center flex-wrap justify-between mt-4">
                ...
                {!isDetailView && <VoteButton poll={poll}
                                              text={(poll.userVote || poll.status == "Archived") ? "View Results" : "Vote"}/>}
                {user && user.role == "admin" && <DeletePollButton poll={poll} text="Delete Poll"/>}
            </div>
        </div>
    );
}
